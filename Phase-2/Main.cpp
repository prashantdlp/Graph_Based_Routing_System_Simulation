...
json process_query(const json &query, Graph &G) 
{
    ...
    if(query["type"] == "k_shortest_paths")
    {
        ...
        auto paths = A.k_shortest_paths(G, source, target, k);
...
int main(int argc, char* argv[]) 
{
    ...
    std::ofstream output_file(out_path, std::ios::out | std::ios::trunc);

    if (!output_file.is_open()) {
        std::cerr << "Failed to create or open file: " << argv[3] << std::endl;
        return 1;
    }
