                    ...
                    const std::vector<double> &speed_profile, bool oneway, const std::string &road_type)
{
    ...
    if (!oneway)
    {
        Edge *reverse_e = new Edge{id, v, u, length, avg_time, speed_profile, oneway, road_type, true};
        adj_list[v].push_back(reverse_e);
        inc_list[u].push_back(reverse_e);
    }
}
...
bool Graph::modifyEdge(int edge_id, const Edge &patch)
{
    ...
    if (patch.oneway.has_value())
        e->oneway = patch.oneway.value();
